...
# Keyed by nonce. Entries have 'status' ('pending' or 'completed'),
# 'token' (GitHub access token, only when completed), and 'timestamp'.
#
# NOTE: In-memory dict is intentional. The desktop app spawns Flask as a
# single child process — there are no multiple workers. The web deployment
# does not use the desktop OAuth flow at all. No need for Redis/DB here.
...
def auth():
    ...
    if source == 'desktop':
        ...
        if not nonce:
            return jsonify({'error': 'nonce parameter is required for desktop auth'}), 400
...
def desktop_status():
    ...
    Called by the Electron app polling loop. If the nonce has a completed
    token, it is moved into the caller's session and the nonce is consumed.
    ...
    Returns:
        ...
        {"error": "nonce_expired_or_invalid"} if nonce is unknown/expired
    """
    nonce = request.args.get('nonce', '')
    if not nonce:
    ...
    entry = _pending_desktop_auths.get(nonce)
    if not entry:
    ...
    # Status is 'completed' — claim the token and consume the nonce
    _pending_desktop_auths.pop(nonce, None)
